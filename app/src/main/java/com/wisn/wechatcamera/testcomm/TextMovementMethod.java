...
public class TextMovementMethod extends LinkMovementMethod {
    ...
    private TextCommentsClickSpan mTextClickSpan;
    ...
    public boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            TextCommentsClickSpan touchTextClickSpan = getTextSpan(widget, buffer, event);
     ...
     * @return
     */
    private TextCommentsClickSpan getTextSpan(TextView widget, Spannable spannable, MotionEvent event) {
        ...
        TextCommentsClickSpan[] link = spannable.getSpans(off, off, TextCommentsClickSpan.class);
        TextCommentsClickSpan touchedSpan = null;
        if (link.length > 0) {
            touchedSpan = link[0];
        }
        return touchedSpan;
